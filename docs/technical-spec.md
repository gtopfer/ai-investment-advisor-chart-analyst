...
> Par técnico do requisito de produto (`docs/product-requirements.md`). Resumo operacional em `system-design.md` (jojo-ai). **o quê** (negócio) + **como** (construção técnica).
...
Ver detalhamento e regras de dependência em `system-design.md` (raiz).
...
**Decisão:** (1) Harness local (`pytest.ini`, ruff nos pacotes do app + `python -m pytest`); (2) parse canônico em `portfolio/import_portfolio.py`; (3) remover BDRs do multiselect (`ASSET_CLASS_OPTIONS`); (4) remover plotly; (5) flag `insufficient_history` + warning UI; (6) `portfolio/candidates.py` + logging em `app.py`.
**Consequências:** Gate de review utilizável; domínio testável sem Streamlit; BDR como classe de filtro fica para feature futura se demandada.
...
**Contexto:** Primeira execução do agente Architect neste repositório. O projeto já possuía código funcional (app Streamlit) antes do processo de agentes, sem stack formal registrada.
**Decisão:** Confirmar a stack já em uso (Python 3.11+, Streamlit, yfinance, pandas/pandas-ta, Groq SDK, pytest, ruff). Documentar o desvio do padrão Clean Architecture: este projeto adota um **Pipeline em Camadas Funcionais** (`data_fetcher → analysis → allocator → ui`, orquestrado por `app.py`), por se tratar de um script Streamlit stateless.
**Alternativas consideradas:** Migrar para Clean Architecture completo → descartado (sem ganho de testabilidade; pipeline já testável por injeção).
**Trade-offs:** simplicidade vs. necessidade futura de API/multiusuário (reabrir decisão se crescer).
**Consequências:** Features seguem `config/ → data_fetcher/ → analysis/ → allocator/ → models/ → ui/ → app.py` e as regras em `system-design.md`.

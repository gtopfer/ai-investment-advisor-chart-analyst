...
def _calculate_macd(price_df: pd.DataFrame) -> str:
...
def _calculate_emas(price_df: pd.DataFrame) -> str:
...
def _calculate_bollinger_bands(price_df: pd.DataFrame) -> str:
...
def _calculate_volatility(price_df: pd.DataFrame) -> float:
...
def _calculate_support_resistance(price_df: pd.DataFrame) -> tuple[float, float]:
    if len(price_df) >= 2:
...
def analyze_chart_patterns(ticker: str, price_df: pd.DataFrame) -> TechnicalIndicators:
    ...
    macd_signal = _calculate_macd(price_df)
    ema_trend = _calculate_emas(price_df)
    bollinger_position = _calculate_bollinger_bands(price_df)
    volatility = _calculate_volatility(price_df)
    support, resistance = _calculate_support_resistance(price_df)
    ...
    return TechnicalIndicators(
        ...
        support_levels=[support] if pd.notna(support) else [],
        resistance_levels=[resistance] if pd.notna(resistance) else []
    )
